...
- **Done:** All five days' teaching content + workflow files are written. Recorded: Day 1 (README §1–7, workflows `01`–`11`), Day 2 (§8–15, `12`–`19`), Day 3 (§16–24, `20`–`29`, ending at the composite action). **Day 4 (§25–30, `30`–`35`)** and **Day 5 (§31–43, `36`–`49` + [day-05/actions/](day-05/actions/): a JS action `greet-js` and a Docker action `greet-docker`)** are scripted, recording pending. YouTube metadata exists for all five days.
- **The day boundaries moved three times; the numbering below is the settled state.** Day 3 was planned to run to the pipeline capstone, but the recording stopped after the composite action (`29`), so token permissions onward moved into `day-04/`. That made Day 4 far too long, so the advanced/security half split out into **Day 5, the finale**. Self-hosted runners then moved back Day 5 → Day 4, to sit just before the production capstone. That last move left a `33a` and a gap at `40`, so files were **renumbered** to close both — the sequence is now unbroken `01`–`49`. The course carries **two capstones**: `35` (production pipeline, closes Day 4) and `49` (hardened pipeline, closes Day 5).
- **Next:** record Day 4 (`30`–`35`) and Day 5 (`36`–`49`); refine content as the videos are shot. No unbuilt topics remain in the outline.
- **Note:** files sit under the folder for the day that teaches them — `day-01/` (`01`–`11`), `day-02/` (`12`–`19`), `day-03/` (`20`–`29`), `day-04/` (`30`–`35`), `day-05/` (`36`–`49`). The numeric prefix is the teaching order.
...
The root README references workflows by relative markdown link (e.g. `[`11-setup-node.yml`](day-01/workflows/11-setup-node.yml)`), so **renaming or renumbering a file means fixing those links** — plus the file's own `# NN -` header comment and `name:` field, and every `(NN)` / `file NN` cross-reference in other workflows' header comments. To slot a topic in between two existing ones, prefer an `04a` style name over renumbering; renumber only for an unrecorded day, and sweep all four reference sites afterwards.

**The numeric prefix is the teaching order.** Numbers run continuously across the whole course: `01`–`11` = Day 1, `12`–`19` = Day 2, `20`–`29` = Day 3, `30`–`35` = Day 4, `36`–`49` = Day 5. The sequence runs **unbroken from `01` to `49`** — no gaps, no letter suffixes. Each file sits under the folder for the day that teaches it: `day-01/workflows/` … `day-05/workflows/`. The numeric prefix, not the folder, remains the source of truth for order — always check the number against the mapping above and against the README.

All five days are written in the README: Day 1 §1–7, Day 2 §8–15 (through status functions), Day 3 §16–24 (job outputs → composite actions), Day 4 §25–30 (token permissions → self-hosted runners → the production capstone), Day 5 §31–43 (SHA pinning → the hardened capstone). **README section numbers and workflow file numbers are different sequences** (§30 is file `35`) — don't assume they line up. **README section numbers are continuous 1–43 and do not restart per day** — the day headings are the only thing that marks a boundary, so moving a topic between days means moving the `# Day N` heading, not renumbering sections.
...
- **35** (the production capstone) calls **27** — a *Day 3* file, in `day-03/workflows/` — so it must be copied alongside, and it needs the `staging` / `production` environments created in repo settings first (see **31**).
...
- **40** / **49** (OIDC) need a cloud IAM role trusting the repo's OIDC `sub` claim, ARN in `vars.AWS_ROLE_ARN`; **34** (self-hosted) sits Queued unless a matching runner is online. These "won't just run" states are deliberate teaching points — check the header before "fixing" them. (**44** / **49** build `sample-app/Dockerfile`, which *is* shipped — those need no extra setup beyond the folder.)

The course also depends on repo-level state a learner must create by hand: the `MY_API_KEY` secret (Day 2, file 14), and the two environments with a required reviewer on `production` (Day 4, files 31 and 35; also needed by 49).
